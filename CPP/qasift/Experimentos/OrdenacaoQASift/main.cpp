...
void runExperiment(QString folder, float q, float b, int from = 0, int to = 0,
  ...
  VideoQASift vid(pattern.toStdString(), from, to, q, b);
  vector<ExperimentFrame> frames;
  for (int i = from; i < to; i += step) {
    ExperimentFrame frame;
    frame.num = i;
    // frame.features = vid.extractASiftFeatures(i);
    frame.features = vid.extractQASiftFeaturesMemoized(i);
...
int main(int argc, char **argv) {
  ...
  parser.addPositionalArgument("q", "Value o q in QGaussian");
  parser.addPositionalArgument("b", "Value o b in QGaussian");
