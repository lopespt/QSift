...
void default_sift_parameters(siftPar &par) {
  ...
  par.EdgeThresh = 0.2; // 0.06;
  par.EdgeThresh1 = 0.42; // 0.08;
  par.OriBins = 36;
  par.OriSigma = 1.5;
  ...
  //	par.MatchRatio = 0.6;
  //	par.MatchRatio = 0.75; // Guoshen Yu. Since l1 distance is used for
  //matching instead of l2, a larger threshold is needed.
  par.MatchRatio = 0.73; // Guoshen Yu. Since l1 distance is used for matching
                         // instead of l2, a larger threshold is needed.
...
void FindMaxMin(flimage *dogs, flimage *blur, float octSize,
...
void InterpKeyPoint(flimage *dogs, int s, int r, int c, const flimage &grad,
                    const flimage &ori, flimage &map, float octSize,
                    keypointslist &keys, int movesRemain, siftPar &par);

void AssignOriHist(const flimage &grad, const flimage &ori, float octSize,
                   float octScale, float octRow, float octCol,
                   keypointslist &keys, siftPar &par);
...
void MakeKeypoint(const flimage &grad, const flimage &ori, float octSize,
                  float octScale, float octRow, float octCol, float angle,
                  keypointslist &keys, siftPar &par);

void MakeKeypointSample(keypoint &key, const flimage &grad, const flimage &ori,
                        float scale, float row, float col, siftPar &par);

void NormalizeVec(float *vec);

void KeySampleVec(keypoint &key, const flimage &grad, const flimage &ori,
                  float scale, float row, float col, siftPar &par);

void KeySample(float index[IndexSize][IndexSize][OriSize], keypoint &key,
               const flimage &grad, const flimage &ori, float scale, float row,
               float col, siftPar &par);

void AddSample(float index[IndexSize][IndexSize][OriSize], keypoint &key,
               const flimage &grad, const flimage &orim, float r, float c,
               float rpos, float cpos, float rx, float cx, siftPar &par);
...
void OctaveKeypoints(flimage &image, float octSize, keypointslist &keys,
  ...
  */
  for (int i = 1; i < par.Scales + 3; i++) {
    ...
    gaussian_convolution(blur[i].getPlane(), blur[i].getPlane(),
                         blur[i].nwidth(), blur[i].nheight(), increase);
...
/////////////////////////////////////////////////
/// Find the local maxima and minima of the DOG images in scale space.  Return
/// the keypoints for these locations.
/////////////////////////////////////////////////
...
/// - NotonEdge:  ratio of the two principle curvatures of the DOG function at
/// this point be below a threshold.
...
void FindMaxMin(flimage *dogs, flimage *blur, float octSize,
  ...
  */
  for (int s = 1; s < par.Scales + 1; s++) {
    ...
    compute_gradient_orientation(blur[s].getPlane(), grad.getPlane(),
                                 ori.getPlane(), blur[s].nwidth(),
                                 blur[s].nheight());
        ...
        */
        ...
        if (fabs(val) > 0.8 * par.PeakThresh) {
          ...
          /*
                                                  ...
                                                  // If local maxima
                                                  if (LocalMax(val, dogs[s-1],
             r, c,par) && LocalMax(val, dogs[s], r, c, par) && LocalMax(val,
             dogs[s+1], r, c,par) && NotOnEdge(dogs[s], r, c, octSize,par))
                                                  {
                                                          if (DEBUG)
             printf("Maximum Keypoint found (%d,%d,%d)  val: %f\n",s,r,c,val);
                                                          InterpKeyPoint(
                                                                  dogs, s, r, c,
             grad, ori, map, octSize, keys, 5,par);

                                                  } else  if (LocalMin(val,
             dogs[s-1], r, c,par) && LocalMin(val, dogs[s], r, c,par) &&
             LocalMin(val, dogs[s+1], r, c,par) && NotOnEdge(dogs[s], r, c,
             octSize,par))
                                                  {
                                                          if (DEBUG)
             printf("Minimum Keypoint found (%d,%d,%d)  val: %f\n",s,r,c,val);
                                                          InterpKeyPoint(
                                                                  dogs, s, r, c,
             grad, ori, map, octSize, keys, 5,par);
                                                  }
          */
          if (LocalMaxMin(val, dogs[s - 1], r, c) &&
              LocalMaxMin(val, dogs[s], r, c) &&
              LocalMaxMin(val, dogs[s + 1], r, c) &&
              NotOnEdge(dogs[s], r, c, octSize, par)) {
            partialcounter++;
            if (DEBUG)
              printf("%d:  (%d,%d,%d)  val: %f\n", partialcounter, s, r, c,
                     val);

            InterpKeyPoint(dogs, s, r, c, grad, ori, map, octSize, keys, 5,
                           par);
...
int NotOnEdge(flimage &dog, int r, int c, float octSize, siftPar &par) {
  ...
  float H00 = dog(c, r - 1) - 2.0 * dog(c, r) +
              dog(c, r + 1), /* AMIR: div by ? */
      H11 = dog(c - 1, r) - 2.0 * dog(c, r) + dog(c + 1, r),
        H01 = ((dog(c + 1, r + 1) - dog(c - 1, r + 1)) -
               (dog(c + 1, r - 1) - dog(c - 1, r - 1))) /
              4.0;
  ...
  /// As we do not desire edges but only corners we demand l_max / l_min less
  /// than a threshold In practice if A = k B,     A*B = k B^2
  ///				(A + B)^2 = (k+1)^2 * B^2
  ///				k B^2 >  t * (k+1)^2 * B^2 sii   k  / (k+1)^2 >
  ///t
...
void InterpKeyPoint(flimage *dogs, int s, int r, int c, const flimage &grad,
  ...
  if (DEBUG)
    printf("peakval: %f, of[0]: %f  of[1]: %f  of[2]: %f\n", peakval, offset[0],
           offset[1], offset[2]);
  ...
  */
  ...
  if (movesRemain > 0 && (newr != r || newc != c)) {
    InterpKeyPoint(dogs, s, newr, newc, grad, ori, map, octSize, keys,
                   movesRemain - 1, par);
    return;
  }
  ...
  if (fabs(offset[0]) > 1.5 || fabs(offset[1]) > 1.5 || fabs(offset[2]) > 1.5 ||
      fabs(peakval) < par.PeakThresh) {
    if (DEBUG)
      printf("Point not well localized by FitQuadratic\n");
    par.noncorrectlylocalized++;
    return;
  }
  ...
  */
  // Guoshen Yu, 2010.09.21 Windows version
  // float octScale = par.InitSigma * pow(2.0, (s + offset[0]) / (float)
  // par.Scales);
  float octScale =
      par.InitSigma * pow(2.0, (s + offset[0]) / (double)par.Scales);

  /// always use histogram of orientations
  // if (UseHistogramOri)
  AssignOriHist(grad, ori, octSize, octScale, r + offset[1], c + offset[2],
                keys, par);
  // else
  //	AssignOriAvg(
...
float FitQuadratic(float offset[3], flimage *dogs, int s, int r, int c) {
  ...
  */
  ...
  return ((*dog1)(c, r) +
          0.5 * (solution[0] * g[0] + solution[1] * g[1] + solution[2] * g[2]));
}

/// - Compute histogram of orientation in a neighborhood weighted by gradient
/// and distance to center
/// - Look for local (3-neighborhood) maximum with valuer larger or equal than
/// par.OriHistThresh * maxval
...
void AssignOriHist(const flimage &grad, const flimage &ori, float octSize,
  ...
  int row = (int)(octRow + 0.5), col = (int)(octCol + 0.5),
      rows = grad.nheight(), cols = grad.nwidth();
  ...
  for (int i = 0; i < par.OriBins; i++) {
    ...
    if (hist[i] > hist[prev] && hist[i] > hist[next] &&
      ...
      if (DEBUG)
        printf("angle selected: %f \t location: (%f,%f)\n", angle, octRow,
               octCol);
      ;
      /* Create a keypoint with this orientation. */
      MakeKeypoint(grad, ori, octSize, octScale, octRow, octCol, angle, keys,
                   par);
    }
  }
...
void KeySample(float index[IndexSize][IndexSize][OriSize], keypoint &key,
  ...
  int irow = (int)(row + 0.5), icol = (int)(col + 0.5);
  float sine = (float)sin(key.angle), cosine = (float)cos(key.angle);
  ...
  for (int i = -iradius; i <= iradius; i++) {
    for (int j = -iradius; j <= iradius; j++) {
      ...
      */
      ...
      if (rx > -1.0 && rx < (float)IndexSize && cx > -1.0 &&
          cx < (float)IndexSize)
        AddSample(index, key, grad, ori, irow + i, icol + j, rpos, cpos, rx, cx,
                  par);
    }
  }
}
...
void AddSample(float index[IndexSize][IndexSize][OriSize], keypoint &key,
  ...
  float sigma = par.IndexSigma * 0.5 * IndexSize,
      ...
      mag =
          weight *
          grad((int)c,
               (int)r); // Guoshen Yu, explicitely cast to int to avoid warning

  /* Subtract keypoint orientation to give ori relative to keypoint. */
  //	float	ori = orim(c,r) -  key.angle;
  float ori = orim((int)c, (int)r) -
              key.angle; // Guoshen Yu, explicitely cast to int to avoid warning
...
void PlaceInIndex(float index[IndexSize][IndexSize][OriSize], float mag,
  ...
  //	int	ri = (rx >= 0.0) ? rx : rx - 1.0,	/* Round down to next
  //integer. */ 		ci = (cx >= 0.0) ? cx : cx - 1.0, 		oi = (oval >= 0.0) ? oval :
  //oval - 1.0;

  int ri = (int)((rx >= 0.0) ? rx : rx - 1.0),
      /* Round down to next integer. */ // Guoshen Yu, explicitely cast to int
                                        // to avoid warning
      ci = (int)((cx >= 0.0) ? cx : cx - 1.0), // Guoshen Yu, explicitely cast
                                               // to int to avoid warning
      oi = (int)((oval >= 0.0) ? oval
                               : oval - 1.0); // Guoshen Yu, explicitely cast to
                                              // int to avoid warning

  float rfrac = rx - ri, /* Fractional part of location. */
      cfrac = cx - ci, ofrac = oval - oi;
...
float DistSquared(keypoint &k1, keypoint &k2, float tdist, siftPar &par) {
  ...
  //	if (abs(k1.x - k2.x) > par.MatchXradius || abs(k1.y - k2.y) >
  //par.MatchYradius) return tdist;

  if (ABS(k1.x - k2.x) > par.MatchXradius ||
      ABS(k1.y - k2.y) > par.MatchYradius)
    return tdist;
  ...
  for (int i = 0; i < VecLength && distsq <= tdist; i++) {
    ...
    //		distsq += ((ik1[i] > ik2[i]) ? (ik1[i] - ik2[i]) : (-ik1[i] +
    //ik2[i]));
  }
...
float DistSquared_short(keypoint_short &k1, keypoint_short &k2, float tdist,
                        siftPar &par) {
  // For Mac/Linux compilation using make: vectorization is possible with short.
  unsigned short distsq = 0;

  // For Windows compilation using Intel C++ compiler: vectorization is possible
  // with int. int distsq = 0;
...
*/
float CheckForMatch(keypoint &key, keypointslist &klist, int &min,
                    siftPar &par) {
  int nexttomin = -1;
...
float CheckForMatch_short(keypoint_short &key, keypointslist_short &klist,
                          int &min, siftPar &par) {
  int nexttomin = -1;
  float dsq, distsq1, distsq2;
  distsq1 = distsq2 = 1000000000000.0f;
...
void compute_sift_matches(keypointslist &keys1, keypointslist &keys2,
  ...
  int imatch = 0;
  float sqminratio = par.MatchRatio * par.MatchRatio, sqratio;
