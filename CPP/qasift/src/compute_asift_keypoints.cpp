...
*/
void qGaussianBlur1D(vector<float>& image, int width, int height, float sigma, int flag_dir, float q, float b)
{
  ...
  */
  ...
  for (i = 0; i <= ksize; i++) {
    ...
    kernel[i] = qGaussianDesvio(x, sigma, q, b);
...
int compute_qasift_keypoints(vector<float>& image, int width, int height, int num_of_tilts, int verb, vector< vector< keypointslist > >& keys_all, siftPar &siftparameters)
...
{	
  float q = siftparameters.qGaussianQ;
  float b = siftparameters.qGaussianB;
...
#pragma omp parallel for private(rr)
      for ( int rr = 1; rr <= num_rot1; rr++ ) 
      {
        ...
        qGaussianBlur1D(image_t,width_r,height_r,sigma_aa,flag_dir,q,b);
