...
Mat VideoQASift::readFrame(const int &frameNum) {
  Mat actualFrame;
  stringstream ss;
...
VideoQASift::VideoQASift(string basename, const unsigned int &from,
                       const unsigned int &to, float q, float b)
    : basename(basename), from(from), to(to), q(q), b(b) {
  default_sift_parameters(siftparameters);
  siftparameters.qGaussianQ = q;
  siftparameters.qGaussianB = b;
}
...
FrameInfo VideoQASift::extractQASiftFeaturesMemoized(const unsigned int &frameNum){
  ...
  return extractQASiftFeatures(frameNum);
}

FrameInfo VideoQASift::extractQASiftFeatures(const unsigned int &frameNum) {
  ...
  num_keys1 =
      compute_qasift_keypoints(fimage, frame.cols, frame.rows, num_of_tilts1,
