...
//#define VIDEO_SRC
//"/Users/wachs/Dropbox/SIFT/Videos/MundoVisto-de-Cima/ScreenCaptureProject108.mp4"
...
vector<FrameInfo> ordena(vector<FrameInfo> list) {
  ...
  while (!list.empty()) {
    ...
    /*auto mitem = min_element(list.begin(), list.end(), [last](const FrameInfo
    &a, const FrameInfo &b) { return PDM(last.features, a.features) <
    PDM(last.features, b.features);
    });*/

    QVector<FrameInfo> qv = QVector<FrameInfo>::fromStdVector(list);

    QVector<FrameGrade> dists =
        QtConcurrent::blockingMapped<QVector<FrameGrade>>(
            qv,
            function<FrameGrade(const FrameInfo &)>([last](const FrameInfo &a) {
              return FrameGrade(PDM(last.features, a.features), a);
            }));

    auto mitem = min_element(dists.begin(), dists.end(),
                             [](const FrameGrade &a, const FrameGrade &b) {
...
float avalia2(vector<FrameInfo> result) {
  ...
  for (int i = 2; i < result.size() && result[i - 1].features.size() > 0 &&
                  result[i].features.size() > 0;
       i++) {
...
vector<FrameInfo> createVectorSpace2(const char *source, int sFrame, int eFrame,
                                     int step, float *q = NULL,
                                     float *b = NULL) {

  VideoSift video(source);
  QVector<int> order =
      QVector<int>::fromStdVector(geraAleatorio(sFrame, eFrame, step));

  vector<FrameInfo> list;
  if (!q) {
    for (auto i : order) {
      list.push_back(FrameInfo{video.extractSiftFeatures(i, 400), i});
    }
  } else {
    for (auto i : order) {
      list.push_back(FrameInfo{video.extractQSiftFeatures(i, 400, *q, *b), i});
    }
  }
...
vector<FrameInfo> createVectorSpace(const char *source, int sFrame, int eFrame,
                                    int step, float *q = NULL,
                                    float *b = NULL) {
  ...
  if (!q || (q && *q == 1)) {
    res = QtConcurrent::blockingMapped<QVector<FrameInfo>>(
        order, function<FrameInfo(const int &)>([&](const int &i) {
          return FrameInfo{video.extractSiftFeatures(i, 100), i};
        }));
  } else {
    res = QtConcurrent::blockingMapped<QVector<FrameInfo>>(
        order, function<FrameInfo(const int &)>([&](const int &i) {
          return FrameInfo{video.extractQSiftFeatures(i, 100, *q, *b), i};
        }));
  }
...
/*
void plotMatlab(Engine *e, vector<double> v, vector<double> x =
vector<double>()) { mxArray *ret = mxCreateDoubleMatrix(v.size(), 1, mxREAL);
...
int main2(int argc, char *argv[]) {
  ...
  QVector<double> v2 = QtConcurrent::blockingMapped<QVector<double>>(
      v, function<double(const int &)>([](const int &i) { return 2.0 * i; }));
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    parser.process(app);
    parser.addOptions(${1 : const QList<QCommandLineOption> & options})
    ...
    tie(from, to, basename) =
        discoverFramesInFolder(parser.positionalArguments().value(0));
    ...
    if (q == 1) {
      ...
      // vector<FrameInfo> captured =
      // createVectorSpace(basename.toStdString().c_str(), from, to, step);
      vector<FrameInfo> captured =
          createVectorSpace(basename.toStdString().c_str(), from, to, step);
      ...
      printf("%.2f\t%.2f\t%d\t%.2f\t%.4f\n", 1.0, b, step,
             countZeroFeatures(ordered), aval2);
      ...
      vector<FrameInfo> captured = createVectorSpace(
          basename.toStdString().c_str(), from, to, step, &q, &b);
